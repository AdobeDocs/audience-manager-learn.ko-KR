...
Adobe Analytics 데이터를 AAM으로 가져오는 두 가지 방법을 비교하고 대조할 때 먼저 다음 이미지에서 차이를 시각화하는 것이 유용할 수 있습니다.
...
위의 이미지에 표시된 대로 히트는 웹 페이지에서 Adobe Analytics으로 이동합니다. 그런 다음 [!DNL Analytics]이(가) 해당 데이터를 실시간으로 AAM에 전달하면 히트가 페이지에서 바로 온 것처럼 방문자가 AAM 트레이트 및 [!UICONTROL segments]&#x200B;(으)로 평가됩니다.

개인화를 위해 웹 페이지에 응답을 전달하는 등의 작업을 수행하는 동일한 실시간 히트에서 [!UICONTROL Segments]이(가) [!DNL Analytics]&#x200B;(으)로 다시 반환됩니다.

서버측 전달로 이동하는 데 있어 시간 단축이 없습니다. Adobe에서는 Audience Manager과 [!DNL Analytics]을(를) 모두 사용하는 모든 사람이 이 구현 메서드를 사용할 것을 강력히 권장합니다.
...
클라이언트측 DIL 코드에서 서버측 전달로 이동할 준비가 되면 첫 번째 단계는 사용자 지정 설정 및 AAM으로 전송된 데이터를 포함하여 DIL 코드로 수행하는 모든 작업을 식별하는 것입니다. 주목하고 고려할 사항은 다음과 같습니다.

* 일반 [!DNL Analytics] 변수, `siteCatalyst.init` DIL 모듈 사용 - 이 변수는 일반적인 [!DNL Analytics] 변수를 전송하는 작업일 뿐이며 서버측 전달을 활성화한 덕택에 발생할 수 있으므로 걱정할 필요가 없습니다.
...
* 추가 페이지 변수 - 다른 변수가 페이지에서 AAM으로 전송되는 경우(siteCatalyst.init에서 처리하는 일반적인 [!DNL Analytics] 변수 외에) 서버측 전달을 통해 전송될 수 있도록 해당 변수를 기록해야 합니다(스포일러 경고: [!DNL contextData] 변수를 통해).
...
#### &quot;페이지에서&quot; 또는 Adobe 이외의 태그 관리자 {#on-the-page-or-non-adobe-tag-manager}

아래 비디오를 통해 클라이언트측 DIL 코드에서 [!DNL AppMeasurement] 코드의 서버측 전달로 구현 옵션을 파일 또는 Adobe 이외의 태그 관리 시스템으로 이동하는 방법에 대해 알아보십시오.

>[!VIDEO](https://video.tv.adobe.com/v/26312/?quality=12)

### 3단계: 전달 활성화([!UICONTROL Report Suite]에 따라) {#step-enabling-the-forwarding-per-report-suite}
...
1. DIL에서 서버 측 전달을 위해 사용할 서버 측 전달 PER [!UICONTROL report suite]로 코드를 업데이트할 준비가 되도록 준비하고 계획합니다.

   1. [!DNL Analytics]에서 스위치를 뒤집어 서버측 전달을 사용하도록 설정합니다.

      1. ECID가 활성화되었으므로 전달이 시작됩니다.

   1. 최대한 빨리 클라이언트측 DIL에서 단일 측 전달로 코드를 업데이트합니다(위의 다른 섹션에서 설명한 대로 Platform 태그나 페이지에서 수행할 수 있음).

      1. [!DNL Analytics] 비콘에 대한 적절한 JSON 응답을 받아야 합니다(자세한 내용은 아래 [유효성 검사 및 문제 해결](#validation-and-troubleshooting) 섹션 참조).

>[!NOTE]
>
>위의 1단계와 2단계 사이에는 AAM으로 이동하는 데이터 중복이 있으므로 이 두 단계를 가능한 한 가깝게 수행하는 것이 중요합니다. 즉, 단일 측 전달이 [!DNL Analytics]에서 AAM으로 데이터를 보내기 시작했으며 DIL 코드가 아직 페이지에 있으므로 페이지에서 바로 AAM으로 이동하는 히트도 발생하여 데이터가 두 배가 됩니다. DIL에서 서버측 전달로 코드를 업데이트하는 즉시 이 기능이 제거됩니다.
...
[!DNL Analytics]에서 Audience Manager으로 데이터의 서버측 전달을 수행하지 않을 경우 [!DNL Analytics] 비콘에 대한 응답(2x2 픽셀 외)이 실제로 없습니다. 그러나 서버측 전달을 수행하는 경우 [!DNL Analytics] 요청 및 응답에서 확인할 수 있는 항목이 있습니다. 이 항목은 [!DNL Analytics]이(가) Audience Manager과 올바르게 통신하고 히트를 전달하며 응답을 받고 있음을 알려줍니다.
